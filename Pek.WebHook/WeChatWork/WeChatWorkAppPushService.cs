...
public static class WeChatWorkAppPushService
{
    ...
    /// <summary>
    /// 发送文本消息给指定企业微信用户，指定应用
    /// </summary>
    /// <param name="appName">应用名称（需先 RegisterApp）</param>
    /// <param name="weComUserId">企业微信UserID（多人用|分隔）</param>
    /// <param name="content">文本内容</param>
    /// <returns>发送结果</returns>
    public static async Task<AppMessageResult> SendTextAsync(String appName, String weComUserId, String content)
    {
        try
        {
            return await WeChatWorkAppClient.SendTextAsync(appName, weComUserId, content);
        }
        catch (Exception ex)
        {
            XTrace.WriteException(ex);
    ...
    /// <summary>
    /// 发送Markdown消息给指定企业微信用户，指定应用
    /// </summary>
    /// <param name="appName">应用名称（需先 RegisterApp）</param>
    /// <param name="weComUserId">企业微信UserID（多人用|分隔）</param>
    /// <param name="content">Markdown内容</param>
    /// <returns>发送结果</returns>
    public static async Task<AppMessageResult> SendMarkdownAsync(String appName, String weComUserId, String content)
    {
        try
        {
            return await WeChatWorkAppClient.SendMarkdownAsync(appName, weComUserId, content);
        }
        catch (Exception ex)
        {
            XTrace.WriteException(ex);
    ...
    /// <param name="btnText">按钮文字</param>
    /// <returns>发送结果</returns>
    public static async Task<AppMessageResult> SendTextCardAsync(String appName, String weComUserId, String title, String description, String url, String? btnText)
    {
        try
        {
            return await WeChatWorkAppClient.SendTextCardAsync(appName, weComUserId, title, description, url, btnText);
        }
        catch (Exception ex)
        {
            XTrace.WriteException(ex);
    ...
    /// <param name="detail">告警详情</param>
    /// <returns>发送结果</returns>
    public static async Task<AppMessageResult> SendAlertAsync(String appName, String weComUserId, String title, String detail)
    {
        var content = $"## ⚠️ {title}\n\n{detail}\n\n> 请及时处理";
        return await SendMarkdownAsync(appName, weComUserId, content);
    }
}
